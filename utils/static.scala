...
val lookupCardinality: Int = sys.env.getOrElse("LOOKUP_CARDINALITY", "10000").toInt


val staticTableLocation = sys.env("STATIC_TABLE_LOCATION")

val staticLookup = spark
 .range(1, lookupCardinality, 1, 100)
 .selectExpr("id AS lookupKey", "id * 2 AS lookupValue")

staticLookup.write
 .format("parquet")
 .mode("overwrite")
 .save(staticTableLocation)
